...
export class ArticlesService {
  ...
  private readonly httpException = new InternalServerErrorException(
  ...
  private async execute<T>(fn: () => Promise<T>): Promise<T> {
    try {
      ...
      console.error('🔴 Caught DB error:', error.message);
  ...
  async findArticle(objectID: string): Promise<ArticleDocument> {
    const article = await this.execute(() =>
      this.articleRepository.findOne(objectID),
    );
  ...
  async createArticle(
    ...
    const exists = await this.execute(() =>
      this.articleRepository.findOne(createArticleDto.objectID),
    );
    if (exists)
      throw new ConflictException(
        `Article #${createArticleDto.objectID} already exists`,
      );
